"""Stair-Descend locomotion environment for Unitree Go2.

Focused single-direction task: flat (20%) + descending stairs (80%).
Use this instead of StairAware when you want to train descending only.

Terrain layout (num_cols=10):
  flat           20%  → cols 0-1  (2 cols)
  descend_tiny   30%  → cols 2-4  (3 cols)  step 0.02–0.06 m, width 0.45 m
  descend_medium 30%  → cols 5-7  (3 cols)  step 0.05–0.15 m, width 0.35 m
  descend_hard   20%  → cols 8-9  (2 cols)  step 0.10–0.25 m, width 0.30 m

Key reward differences vs StairAware:
  - NO ascend_forward_progress  (wrong direction signal for descend)
  - NO stair_height_progress    (would penalise downward movement)
  - descend_stable_progress with very relaxed pitch_weight (0.02)
    so the policy is not harshly penalised for the forward lean that
    naturally occurs when descending stairs.
"""
...
STAIR_DESCEND_TERRAIN_CFG = terrain_gen.TerrainGeneratorCfg(
    size=(8.0, 8.0),
    border_width=20.0,
    num_rows=10,
    num_cols=10,
    ...
    sub_terrains={
        ...
        # ── Descending stairs (80%) ────────────────────────────────────────
        # Pyramid: center is the highest point, edges are low.
        # Robot spawns at center (top) and walks outward → descends DOWN.
...
class RobotSceneCfg(InteractiveSceneCfg):
    terrain = TerrainImporterCfg(
        ...
        terrain_generator=STAIR_DESCEND_TERRAIN_CFG,
...
@configclass
class CommandsCfg:
    # Stair descend: forward-only commands.
    # Lateral and angular velocity fixed at 0 — side-stepping or turning
    # on stairs is unnatural and adds unnecessary exploration difficulty.
    # Speed curriculum (lin_vel_cmd_levels_descend) can still raise x upper bound.
...
class RewardsCfg:
    ...
    base_linear_velocity = RewTerm(
        func=mdp.lin_vel_z_up, weight=-3.0,  # penalize only upward z motion during descend
        params={"asset_cfg": SceneEntityCfg("robot")},
    )
    # Very relaxed pitch penalty: going down stairs involves natural forward
    # lean; harshly penalising pitch causes the policy to halt at step edges.
    roll_pitch_angular_velocity = RewTerm(
        func=mdp.roll_pitch_ang_vel_l2,
        weight=-0.05,
        params={"pitch_weight": 0.05, "roll_weight": 1.0},
    )
    ...
    feet_gait = RewTerm(
        func=mdp.feet_gait,
        weight=0.2,  # reduced from 0.5 — trot pattern too rigid for medium/hard descent
        params={
            "period": 0.5,
    ...
    # Descending-specific: reward forward progress + stability on descend cols.
    # flat_cols=2 (cols 0-1), ascend_cols=0 (no ascend), so all stair cols
    # (2-9) are treated as descend terrain.
    descend_stable_progress = RewTerm(
        func=mdp.descend_stable_progress,
        weight=2.5,   # strengthened: main signal for controlled descent
        params={
            "asset_cfg": SceneEntityCfg("robot"),
            "flat_cols": 2,
            "ascend_cols": 0,
            "pitch_weight": 0.02,   # very relaxed — faceplant prevention only
            "roll_weight": 1.0,
        },
    )
...
class CurriculumCfg:
    ...
    terrain_levels = CurrTerm(func=mdp.terrain_levels_two_axis_descend)
    lin_vel_cmd_levels = CurrTerm(mdp.lin_vel_cmd_levels_descend)
...
class RobotEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        self.decimation = 4
        self.episode_length_s = 15.0  # shortened: prevents robot from reaching adjacent ascending terrain after descent
        self.sim.dt = 0.005
